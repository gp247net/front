{{--
    ...
    Variables: $rows (FrontLinkGroup paginator).
--}}
<div>
    <x-gp247::list-toolbar :placeholder="gp247_language_render('admin.core.search')"
        :selected-count="count($selected)" :bulk-confirm="gp247_language_render('action.delete_confirm')">
        <x-slot:actions>
            <x-gp247::button href="{{ gp247_route_admin('admin_link_group.create') }}" wire:navigate size="sm">
                <i class="fas fa-plus"></i> {{ gp247_language_render('admin.link_group.add_new') }}
            </x-gp247::button>
        </x-slot:actions>
    ...
    <x-gp247::table :empty="$rows->isEmpty() ? gp247_language_render('admin.core.no_records') : null">
        <x-slot:head>
            <tr>
                ...
                <x-gp247::th-sort field="code" :sort-field="$sortField" :sort-dir="$sortDir">{{ gp247_language_render('admin.link_group.code') }}</x-gp247::th-sort>
                <x-gp247::th-sort field="name" :sort-field="$sortField" :sort-dir="$sortDir">{{ gp247_language_render('admin.link_group.name') }}</x-gp247::th-sort>
        ...
        @foreach ($rows as $row)
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50" wire:key="link-group-{{ $row->id }}">
                ...
                <td class="px-4 py-3">
                    <x-gp247::row-actions
                        :edit="gp247_route_admin('admin_link_group.edit', ['id' => $row->id])"
                        :delete-id="$row->id"
