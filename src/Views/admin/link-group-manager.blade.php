{{--
    Link-group manager (front-admin Unit) — two-panel: add/edit form (left) +
    ...
    Variables: $rows (FrontLinkGroup paginator); $form, $editingId,
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
    ...
    <x-gp247::card :title="gp247_language_render($editingId ? 'action.edit' : 'admin.link_group.add_new')">
        <form wire:submit="save" class="space-y-4">
            ...
            <x-gp247::input
                :label="gp247_language_render('admin.link_group.name')"
                name="name"
            ...
            <x-gp247::input
                :label="gp247_language_render('admin.link_group.code')"
                name="code"
    ...
    {{-- Right: list --}}
    <x-gp247::card :title="gp247_language_render('admin.link_group.title')">
            ...
            <x-slot:head>
                <tr>
                    <th class="cursor-pointer px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400"
                        ...
                        {{ gp247_language_render('admin.link_group.code') }}
                        @if ($sortField === 'code')<span class="text-[10px]">{{ $sortDir === 'asc' ? '▲' : '▼' }}</span>@endif
                    </th>
                    <th class="cursor-pointer px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400"
                        wire:click="setSort('name')">
                        {{ gp247_language_render('admin.link_group.name') }}
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-end gap-1">
                            <x-gp247::button size="sm" variant="ghost"
                                href="{{ gp247_route_admin('admin_link_group.edit', $row->id) }}"
                                wire:navigate>
